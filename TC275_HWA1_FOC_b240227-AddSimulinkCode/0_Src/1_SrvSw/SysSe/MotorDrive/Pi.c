...
{
    Pic->a0 = 0;//-0.0002577558
    Pic->a1 = 0;//0.000257931
    Pic->ik = 0;
    Pic->uk = 0;
    Pic->min = 0;
    Pic->max = 0;
...
void SpeedPI_Init(TPic * Pic)
{
    /*set KI*/
    Pic->b0 = 0;//0.0001//0.005//0.0001

    /*set KP*/
    Pic->b1 = 0.000001;//0.02//0.005//0.001
...
{
    /*set KI*/
    Pic->b0 = 0.0001;//0.0001//0.005//0.0001

    /*set KP*/
    Pic->b1 = 0.01;//0.02//0.005//0.001
...
/** @brief Set the PI controller Kp and Ki parameter (for fix point calculation).
 *
 * This function sets the PI controller Kp and Ki parameter. The A0 and A1 coefficient
 ...
 * @param ShiftResult Shift done on the result for fix point calculation. Range = [-32, 31]. In case TStdReal type is float32, this parameter is ignored
 *
 * @return none
...
{
    StdRealLong uk;
//    Uk = A1 * Input + Uk + A0 * Pic->Ik;//A1=kp*1.00034;A0=-kp*0.99966
#if (STD_REAL_TYPE == STD_REAL_FIX_POINT_16) || (STD_REAL_TYPE == STD_REAL_FIX_POINT_32)
    ...
    pic->delta_uk = __maddms_rLSR_iSR_iSR_iSR_iSR(pic->a1, input, pic->a0, pic->ik);
#endif
...
StdReal Speed_PI(TPic * pic, StdReal input)
{
    StdRealLong uk;

    uk = __madds_rF_iF_iF_iF(__maddms_rLSR_iSR_iSR_iSR_iSR(pic->b1, input, pic->b0, pic->ik) , pic->c0 , pic->sk);
