...
IFX_EXTERN VelocityControl g_velocityControl;
...
IFX_INLINE void VelocityControl_Reset(VelocityControl *velocityControl)
{
    ...
    velocityControl->delay_count = 50;
...
IFX_INLINE boolean VelocityControl_SetRefSpeed(VelocityControl *velocityControl, float32 speed)
{
    ...
    velocityControl->delay_count++;

    if( velocityControl->delay_count >= 50)
    {
        if((__absf(speed)) > (velocityControl->max))
        {
            result = FALSE;
        }
        else if ((speed - velocityControl->ref) > 500)
        {
            velocityControl->ref += 500;
            result = TRUE;
        }
        else
        {
