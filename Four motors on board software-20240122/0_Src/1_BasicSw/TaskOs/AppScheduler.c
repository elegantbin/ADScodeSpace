...
void AppScheduler_init(void)
{
    // Initialize AppScheduler Tasks
        ...
        config = appScheduler_task_config[AppScheduler_Task_2ms];
        config.callback = (schedulerCallback) &appTaskfu_2ms;
        AppScheduler_initTask(&g_tasks_core0[AppScheduler_Task_2ms],&config);
}
