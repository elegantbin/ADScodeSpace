...
void initGtmATomPwm_6Step(void)
{
    ...
    //V鐩�
    {
        ...
        g_atomConfig[V].interrupt.isrPriority = ISR_PRIORITY_ATOM_PWM_VPHASE;
        g_atomConfig[V].interrupt.mode = IfxGtm_IrqMode_pulseNotify;
...
IFX_STATIC void setDutyCycle(Phase phase, uint16 dutyCycle)
{
    //V U W
    //0 1 2
    g_atomConfig[phase].dutyCycle = dutyCycle * PWM_PERIOD / 10000;                 /* Set duty cycle        */
...
void HoldingPosition(void)
{
    ...
    IfxGtm_Atom_Pwm_stop(&g_atomDriver[V], TRUE);
    IfxGtm_Atom_Pwm_stop(&g_atomDriver[U], TRUE);
    IfxGtm_Atom_Pwm_stop(&g_atomDriver[W], TRUE);
