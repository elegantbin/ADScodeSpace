...
sint16 Encoder_SPIPosition;
sint16 SPIEncoderInitAngle(void)
{
    Ifx_GPT12 *gpt12=&MODULE_GPT120;

    transferData();
    Encoder_SPIPosition = (g_qspi.spiBuffers.spiSlaveRxBuffer[0] & 0x3FFF);
    Encoder_SPIPosition = Encoder_SPIPosition>>2;
    Encoder_SPIPosition %=K_ENCODER_TOTAL_COUNTS_PER_MECH_CIRCLE;
    Encoder_SPIPosition =K_ENCODER_TOTAL_COUNTS_PER_MECH_CIRCLE-Encoder_SPIPosition;

    IfxCpu_disableInterrupts();
    gpt12->T2.U=Encoder_SPIPosition;
    IfxCpu_enableInterrupts();

    return Encoder_SPIPosition;
}
